...
loss and higher validation accuracy than SGD within the same 5 epochs.
SGD was still improving at the end of training, indicating that it would
likely need more epochs to match Adam's performance on this task.
...
\textit{Inference:} LeNet-5's training and validation accuracy rise
slowly and track each other closely, with no visible overfitting, but the
final accuracy plateaus around 54\%, reflecting the model's limited
representational capacity (only 83,126 parameters) for a 10-class colour
image dataset.
...
\textit{Inference:} LeNet-5 confuses visually similar classes such as
cat/dog and automobile/truck most often, and performs relatively well on
...
\textit{Inference:} AlexNetMini shows a steady, well-behaved increase in
accuracy and decrease in loss, reaching around 75\% training and
validation accuracy by epoch 12 with almost no gap between the two
curves, indicating good generalisation for a from-scratch model.
...
\textit{Inference:} AlexNetMini achieves strong diagonal dominance across
most classes, with the main confusion remaining between cat, dog and bird,
which are the classes with the greatest intra-class visual variability.
...
\textit{Inference:} Training for ResNetMini was stopped early (after 4 of
12 epochs) by the \texttt{EarlyStopping} callback once validation loss
stopped improving; the restored ``best'' weights nonetheless generalised
poorly to the test set (47.26\% accuracy), suggesting the small residual
model was not deep/regularised enough to be trained stably in so few
epochs.
...
a few classes (e.g. automobile, frog, ship), with the truck class almost
never predicted correctly (recall $\approx$ 0.03), confirming the unstable,
under-trained behaviour seen in the accuracy/loss curves.
...
accuracy within 8 epochs, far exceeding all from-scratch baselines,
demonstrating the value of ImageNet-pretrained features even without
fine-tuning.
...
classes, with the largest remaining confusion between cat and dog, which
remains the hardest pair to separate throughout this experiment.
...
accuracy climbs sharply to 96\% while validation accuracy reaches about
91\%, a visible generalisation gap indicating mild overfitting, but the
validation accuracy is still well above the frozen-backbone result.
...
\textit{Inference:} Most misclassified images involve genuine visual
ambiguity (e.g. a deer photographed against a similarly coloured
background, a cat mistaken for a dog, or a car partly occluded), showing
that many of the remaining errors are close to the irreducible difficulty
of the $32\times32$ CIFAR-10 images rather than model deficiencies.
...
\textit{Inference:} ResNet50's frozen backbone converges even faster than
VGG16's, reaching about 86-87\% validation accuracy within just 4 epochs,
reflecting the strength of its residual ImageNet features.
...
\textit{Inference:} The frozen ResNet50 model shows a well-balanced
confusion matrix with high recall across most classes; cat and dog remain
the most confused pair, consistent with the VGG16 results.
...
\textit{Inference:} Fine-tuning the last 10 layers of ResNet50 yields a
smaller additional gain than for VGG16, as the training and validation
curves rise only modestly beyond the frozen-stage result, suggesting
ResNet50's frozen features were already close to their achievable
performance on CIFAR-10 at this image resolution.
...
backbones, but the effect is highly asymmetric: VGG16 gains substantially
from fine-tuning (83.10\% $\rightarrow$ 90.26\%) and overtakes ResNet50,
whereas ResNet50's frozen accuracy was already higher than VGG16's and it
gains comparatively little from fine-tuning (84.41\% $\rightarrow$
86.14\%).
...
\textit{Note:} GoogleNet/Inception was not trained in this run; VGG16 and
ResNet50 were selected as the two representative backbones for the
transfer-learning tasks, as permitted by the Task~2 instructions.
Parameter counts for ResNet50 were not captured by \texttt{model.summary()}
in this notebook run and are therefore marked ``--''.

\textit{Overall inference:} Transfer learning clearly outperforms
training small CNNs from scratch on CIFAR-10 -- even the frozen VGG16
and ResNet50 backbones (83--84\% accuracy) beat the best from-scratch
baseline, AlexNetMini (75.26\%), by a wide margin, while requiring
comparable or less training time. Fine-tuning the last convolutional
block of the pretrained backbones improves accuracy further, with VGG16
Fine-Tuned achieving the best overall test accuracy (90.26\%) in this
experiment.
